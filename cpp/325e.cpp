 ...
 for (int i = 0; i < n; i++) {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < i; j++) {
        ...
        int conU = (u * 2) - 2; 
        int conV = (v * 2) - 2;
        ll time = (ll)cost[i][j]*a;
        //ここは双方向の辺
        g[conU].emplace_back(conV, time); //移動のコストは1
        g[conV].emplace_back(conU, time); //移動のコストは1
        
        //電車のコスト
        int revU = (u * 2) - 1; 
        int revV = (v * 2) - 1;
        time = (ll)cost[i][j]*b + c;
        //ここは双方向の辺
        g[revV].emplace_back(revU, time); //移動のコストは1
        g[revU].emplace_back(revV, time); //移動のコストは1
    }
  }
  /*for (int i = 0; i < m; i++) {
      int u, v;
